...
async def test_signup_agent_success(async_client: AsyncClient):
    ...
    assert response.json()["email"] == "new.agent@test.com"
...
async def test_change_password(async_client: AsyncClient, db_session: AsyncSession, owner_user: User):
    response = await async_client.put("/api/auth/change-password/", json={
        "old_password": "password123",
        "new_password": "NewPassword123!"
    })
    assert response.status_code == 200
    
    # Reload user and verify password change
    await db_session.refresh(owner_user)
    assert verify_password("NewPassword123!", owner_user.password_hash)
